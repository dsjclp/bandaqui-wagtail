...
from events.managers import CategoryManager
...
class InstrumentEventPage(models.Model):
    ...
    instrument = models.ForeignKey(Instrument, related_name="+", verbose_name=_('Instrument'), on_delete=models.CASCADE)
    page = ParentalKey('EventCalPage', related_name='event_instruments')
    nbmax = models.IntegerField(default=99)
    panels = [
        FieldPanel('instrument'),
        FieldPanel('nbmax')
    ]

    def __str__(self):
        return '%s %s %s' % (self.page,self.instrument,self.nbmax)